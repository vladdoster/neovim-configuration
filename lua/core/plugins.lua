...
packer.startup {
  function(use)
    ...
    use { "karb94/neoscroll.nvim", event = "BufRead", config = [[require("configs.neoscroll").config()]] }
    ...
    use { "L3MON4D3/LuaSnip", config = [[require("luasnip/loaders/from_vscode").lazy_load()]], requires = { "rafamadriz/friendly-snippets" } }
    ...
    use { "neovim/nvim-lspconfig", event = "BufRead", config = [[require "configs.lsp"]] }
    use { "tami5/lspsaga.nvim", event = "BufRead", config = [[require "configs.lsp.lspsaga".config()]] }
    use { "simrat39/symbols-outline.nvim", cmd = "SymbolsOutline", setup = [[require "configs.symbols-outline".setup()]] }
    ...
    use { "norcalli/nvim-colorizer.lua", event = "BufRead", config = [[require "configs.colorizer".config()]] }
    use { "windwp/nvim-autopairs", event = "InsertEnter", config = [[require "configs.autopairs".config()]] }
    use { "akinsho/nvim-toggleterm.lua", cmd = "ToggleTerm",config = [[require("configs.toggleterm").config()]] }
    use { "numToStr/Comment.nvim", event = "BufRead", config = [[require "configs.comment".config()]] }
    use { "lukas-reineke/indent-blankline.nvim", config = [[require("configs.indent-line").config()]] }
    use { "folke/which-key.nvim", config = [[require("configs.which-key").config()]] }
    use {
