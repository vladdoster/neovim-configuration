...
---@return string
function _G.get_runtime_dir() return vim.fn.stdpath 'data' end

---Get the full path to `$LUNARVIM_CONFIG_DIR`
---@return string
function _G.get_config_dir() return vim.fn.stdpath 'config' end

---Get the full path to `$LUNARVIM_CACHE_DIR`
---@return string
function _G.get_cache_dir() return vim.fn.stdpath 'cache' end
...
function M:init(base_dir)
    ...
    ---@return string
    function _G.get_lvim_base_dir() return base_dir end
...
function M:get_version(type)
    ...
    if not is_on_master then
        ...
        if not log_status_ok or string.match(abbrev_version, '%d') == nil then return nil end
    ...
    if not tag_status_ok or string.match(lvim_full_ver, '%d') == nil then return nil end
    if type == 'short' then
