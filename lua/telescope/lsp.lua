...
local M = {}
M.document_diagnostics = function() builtin.diagnostics {bufnr=0} end
M.workspace_diagnostics = function() builtin.diagnostics() end
M.definitions = function() builtin.lsp_definitions() end
M.references = function() builtin.lsp_references() end
M.buffer_references = function(opts)
  ...
  vim.lsp.buf_request(opts.bufnr, 'textDocument/references', params, function(err, result, ctx, _config)
    ...
    if result then
      ...
      filtered_result = vim.tbl_filter(function(location) return (location.uri or location.targetUri) == buf_uri end,
                                       result)
      locations = vim.lsp.util.locations_to_items(filtered_result,
                                                  vim.lsp.get_client_by_id(ctx.client_id).offset_encoding) or {}
    end
...
M.implementations = function() builtin.lsp_implementations() end
